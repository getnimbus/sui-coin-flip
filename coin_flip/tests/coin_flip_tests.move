...
module nimbus::coin_flip_tests {
    ...
    fun test_e2e() {
        ...
        debug::print(&round);
        let result= coin_flip::flip(&mut round, 0, x"b55e7cb2d5c613ee0b2e28d6750aabbb78c39dcc96bd9d38c2c2e12198df95571de8e8e402a0cc48871c7089a2b3af4b");
        debug::print(&result);

        scenario.next_tx(user1);
        debug::print(&round);
        let result = coin_flip::flip(&mut round, 0, x"b6b6a585449b66eb12e875b64fcbab3799861a00e4dbf092d99e969a5eac57dd3f798acf61e705fe4f093db926626807");
        debug::print(&result);

        // User2 is the winner since the mod of the hash results in 1
        scenario.next_tx(user2);
        debug::print(&round);
        let result = coin_flip::flip(&mut round, 0, x"b3fab6df720b68cc47175f2c777e86d84187caab5770906f515ff1099cb01e4deaa027075d860823e49477b93c72bd64");
        debug::print(&result);
